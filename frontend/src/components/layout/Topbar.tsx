...
import { Menu, ChevronDown, Check, Building2, MapPin, Car } from 'lucide-react';
...
export const Topbar: React.FC<TopbarProps> = ({ onOpenMobileMenu }) => {
  const { sites, currentSiteId, currentSite, setCurrentSiteId, availability } = useLiveData();
  ...
  return (
    <header className="sticky top-0 z-20 bg-white border-b border-neutral-200 shadow-soft">
      <div className="max-w-[1440px] mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-4">
        {/* Left: Mobile hamburger & Site Switcher */}
        <div className="flex items-center gap-3">
          ...
          {/* Site Switcher Control per §3.2.2 */}
          <div className="relative" ref={dropdownRef}>
            ...
            {isSiteDropdownOpen && (
              <div className="absolute left-0 top-full mt-2 w-72 bg-white rounded-card shadow-modal border border-neutral-200 py-1.5 z-50 animate-scale-up">
                <div className="px-3 py-2 border-b border-neutral-100">
                  <p className="text-[11px] font-bold uppercase tracking-wider text-neutral-400">Switch Physical Site</p>
                </div>
                <div className="max-h-60 overflow-y-auto py-1 divide-y divide-neutral-50">
                  {sites.map((s) => {
